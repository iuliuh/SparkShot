...
namespace Utils
{
	...
	HotKey stringToHotKey(const QString& key)
	{
		HotKey hotKey;

		QStringList keyList = key.split(KEY_SEPARATOR);

		const QString keyString = keyList.takeLast();
		const QStringList modifierStrings = keyList;

		Q_FOREACH(QString modifierString, modifierStrings)
		{
			// Convert modifier
			qDebug() << "Modifier: " << modifierString;
		}
